...
// Demo: Save photos and notes in browser localStorage
...
const getSavedNotes = () => localStorage.getItem('inspectionNotes') || '';
...
const InspectionTool = () => {
  ...
  const [photos, setPhotos] = useState(getSavedPhotos());
  ...
  // Save photo to localStorage and update checklist
  ...
  const startCamera = async () => {
    try {
      ...
      setCameraError('Unable to access camera. Please check permissions or hardware.');
  ...
  const analyzePhoto = async (base64, itemId) => {
    ...
    try {
      ...
      if (!resp.ok) throw new Error('AI assessment failed');
      const { findings: aiFindings } = await resp.json();
  ...
  const saveNotes = () => {
    localStorage.setItem('inspectionNotes', notes);
    setInfoMsg('Notes saved!');
    setTimeout(() => setInfoMsg(''), 2000);
  };
  ...
  const CameraTab = () => (
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        {cameraError && <div className="bg-red-100 text-red-700 px-4 py-2 rounded">{cameraError}</div>}
        ...
        <div className="flex space-x-2">
          {!cameraActive ? (
            <button onClick={startCamera} className="bg-green-600 text-white px-4 py-2 rounded-lg">Start Camera</button>
          ) : (
            <>
              <button onClick={stopCamera} className="bg-red-600 text-white px-4 py-2 rounded-lg">Stop Camera</button>
            </>
          )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <button
          onClick={saveNotes}
          className="mt-2 bg-green-600 text-white px-4 py-2 rounded-lg flex items-center"
        >
                      ...
                      <div className="flex items-center space-x-2">
                        {photos[item.id]?.length > 0 && (
                          <span className="bg-green-100 text-green-800 px-2 py-1 rounded text-xs">
                            {photos[item.id].length} photo{photos[item.id].length > 1 ? 's' : ''}
                          </span>
                        )}
                        <button
                        ...
                        {photos[item.id]?.map((src, idx) => (
                          <img key={idx} src={src} alt="Captured" style={{width: 40, marginLeft: 4, borderRadius: 4}} />
                        ))}
                        {/* Optionally add AI analysis button for last photo */}
                        {photos[item.id]?.length > 0 && (
                          <button
                            disabled={loadingAI}
                            onClick={() => analyzePhoto(photos[item.id][photos[item.id].length - 1], item.id)}
                            className="bg-purple-600 text-white px-3 py-1 rounded text-sm flex items-center"
                          >
                            {loadingAI ? "Analyzing..." : "AI Analyze"}
                          </button>
                        )}
                        {/* Show AI findings if available */}
                        {findings[item.id] && (
  ...
  return (
    <div className="space-y-6">
      {infoMsg && (
        <div className="bg-blue-100 text-blue-700 px-4 py-2 rounded">{infoMsg}</div>
      )}
      {errorMsg && (
